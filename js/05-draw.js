    ...
    // ---- свободный поворот слоя с чистыми гранями (RotSprite) ----
    const cellInt = (c) => c ? ((((c[0] << 24) | (c[1] << 16) | (c[2] << 8) | (c.length > 3 ? c[3] : 255)) >>> 0)) : 0;
    const intCell = (v) => v ? [(v >>> 24) & 255, (v >>> 16) & 255, (v >>> 8) & 255, v & 255] : null;
    function layerToInt(L) { const src = new Int32Array(W * H);
      for (let y = 0; y < H; y++) for (let x = 0; x < W; x++) src[y * W + x] = cellInt(L.grid[y][x]); return src; }
    function buildRotPreview(L, ang, scale) { const r = rotSprite(layerToInt(L), W, H, ang, scale);
      ...
      return { canvas: c, px: Math.round((W - r.w) / 2), py: Math.round((H - r.h) / 2), ow: r.w, oh: r.h }; }
    function freeRotateLayer(L, ang, scale) { const r = rotSprite(layerToInt(L), W, H, ang, scale);
      snapshot(); L.grid = blank(W, H); L.ext = new Map();
      const tx = Math.round((W - r.w) / 2), ty = Math.round((H - r.h) / 2);
