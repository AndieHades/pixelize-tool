...
import { shadingActive, shadingStamp } from './shading.js';

export function stamp(x, y) {
  if (S.tool === 'select' || S.tool === 'move') return;
  if (S.tool === 'pencil' && shadingActive()) shadingStamp(x, y);
  else if (S.tool === 'pencil' || S.tool === 'line' || S.tool === 'rect' || S.tool === 'ellipse') brushStamp(x, y, false);
