...
export function doAddLayer() { if (S.layers.length >= MAX_LAYERS) { toast('Максимум 8 слоёв'); return; }
...
export function doMerge() { let idx = [...S.marked].sort((a, b) => a - b);
  if (idx.length < 2) { if (S.cur > 0) idx = [S.cur - 1, S.cur]; else { toast('Отметь 2+ слоя галочками'); return; } }
  snapshot();
  ...
  S.layers.splice(base, 0, merged); S.cur = base; S.marked.clear(); dirtyAll(); bus.emit('layers'); bus.emit('render'); toast('Слои слиты'); }
...
export function doGroup() { let idx = [...S.marked].sort((a, b) => a - b); if (!idx.length) idx = [S.cur];
  ...
  S.cur = idx[0] + moved.length - 1; S.marked.clear(); dirtyAll(); bus.emit('layers'); bus.emit('render'); toast('Папка создана'); }

export function duplicateLayer(L) { if (S.layers.length >= MAX_LAYERS) { toast('Максимум 8 слоёв'); return; }
  const idx = S.layers.indexOf(L); if (idx < 0) return; snapshot();
  S.layers.splice(idx + 1, 0, { name: L.name + ' копия', opacity: L.opacity, visible: L.visible, fid: L.fid, clip: !!L.clip, symLock: !!L.symLock, ext: new Map(L.ext), grid: cloneGrid(L.grid) });
  S.cur = idx + 1; S.marked.clear(); dirtyAll(); bus.emit('layers'); bus.emit('render'); toast('Слой продублирован'); }

export function duplicateFolder(f) { const kids = folderLayers(f); if (!kids.length) return;
  if (S.layers.length + kids.length > MAX_LAYERS) { toast('Максимум 8 слоёв'); return; }
  ...
  const dst = topOfFolder(f.id) + 1; S.layers.splice(dst, 0, ...copies); S.cur = dst + copies.length - 1; S.marked.clear(); dirtyAll(); bus.emit('layers'); bus.emit('render'); toast('Папка продублирована'); }

export function deleteLayer() { if (S.layers.length < 2) { toast('Это единственный слой'); return; }
