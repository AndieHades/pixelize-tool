...
export function liftSelection() { const L = S.layers[S.cur], g = L.grid, w = S.sel.x1 - S.sel.x0 + 1, h = S.sel.y1 - S.sel.y0 + 1;
  ...
  S.selMask = null; S.selFloat = { cells, w, h, x: S.sel.x0, y: S.sel.y0, ox: S.sel.x0, oy: S.sel.y0 }; markDirty(S.cur); }
...
export function liftSelectionSym(grabX, grabY) { const L = S.layers[S.cur], g = L.grid, gL = grabX * 2 <= S.W - 1, gT = grabY * 2 <= S.H - 1, items = [];
  ...
  S.selMask = null; S.selFloat = { symItems: items, dx: 0, dy: 0 }; markDirty(S.cur); }
...
export function commitFloat() { if (!S.selFloat) return; const L = S.layers[S.cur], g = L.grid, landed = new Set();
  ...
  S.selMask = landed.size ? landed : null; S.selFloat = null; markDirty(S.cur); bus.emit('layers'); }
