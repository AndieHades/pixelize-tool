...
function colApply() { S.active = hsvToRgb(colH, colS, colV); $('picker').value = rgbToHex(S.active); bus.emit('palette'); bus.emit('render'); }
...
export function mount() {
  ...
  $('col-add').onclick = () => { const c = hsvToRgb(colH, colS, colV); if (S.palette.some((p) => eqc(p, c))) return;
    S.palette.push(c.slice()); S.active = c.slice(); bus.emit('palette'); bus.emit('render'); };
